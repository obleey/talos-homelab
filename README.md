...
kubeseal -f /tmp/my-secret.yaml \
  -w deployments/infrastructure/my-app/my-secret-sealed.yaml
...
```bash
git clone https://github.com/your-username/talos-homelab
cd talos-homelab
```
...
├── my-app-values.yaml    ← Helm values
├── httproute.yaml        ← Gateway API route
└── secret-sealed.yaml    ← Any sealed secrets
```
...
spec:
  ...
  sources:
    ...
    - repoURL: https://github.com/your-username/talos-homelab
      targetRevision: HEAD
      ref: values
...
Repo: https://github.com/your-username/talos-homelab
Error: <error details>
```
...
# Seal a new secret
kubeseal -f /tmp/secret.yaml -w deployments/path/to/sealed-secret.yaml
